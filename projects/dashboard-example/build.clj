...
(println "=== Building Dashboard Example ===\n")

;; Define elements as a collection (not nested)
(def elements [
  ;; Header
  [:header {:class "header"}
   [:h1 {} "Dashboard"]
...
(println "Compiling " (count elements) " elements...")
(def compiled-html (compiler/compile-to-html elements context))
(def compiled-html-string (compiler/html-output->string compiled-html))

(println "\n=== Compiled HTML (first 1000 chars) ===")
(println (subs compiled-html-string 0 (min 1000 (count compiled-html-string))))
(println "...")

;; Check for HTMX attributes
(if (clojure.string/includes? compiled-html-string "hx-get")
...
(println (str "✓ Total compiled size: " (count compiled-html-string) " characters"))
