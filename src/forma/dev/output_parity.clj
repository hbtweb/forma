...
(defn test-compile-to-html
  ...
  (let [results (atom [])]
    ...
    ;; Test multiple elements (compile-to-html handles vector of elements)
    (try
      (let [output (compiler/compile-to-html [[:div {} "Content"]])
            html (compiler/html-output->string output)]
        (if (and (string? html) (> (count html) 0))
          (swap! results conj (pass "compile-to-html handles elements" html))
          (swap! results conj (fail "compile-to-html" output "non-empty HTML"))))
      (catch Exception e
        (swap! results conj (fail "compile-to-html" (.getMessage e) "no exception"))))
